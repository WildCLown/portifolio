<template>
  <v-row
    ...
    @click.capture="reset" @input.capture="reset" @change.capture="reset"
  >
    ...
    <v-col cols="2" class="text-start bt-3">
      <v-row
        no-gutters
        class="justify-start "
    ...
    methods: {
        ...
        CdoPoint(){
            if(!this.doPoint){//Modificando botão e controle para 
                document.getElementById("criar").innerHTML = "Finish Control Placement";
                this.doPoint = true;
            }else{//Modifica botão, controle e coloca o vetor de pontos em outro vetor (matriz)
                document.getElementById("criar").innerHTML = "New Curve"; 
                this.doPoint = false;
